...
const RoomDetailsPage = () => {
  ...
  const nextImage = () => {
    if (roomDetails.roomPhotoUrls && roomDetails.roomPhotoUrls.length > 0) {
      setCurrentImageIndex((prevIndex) => 
        prevIndex === roomDetails.roomPhotoUrls.length - 1 ? 0 : prevIndex + 1
      );
    }
  };
  ...
  const prevImage = () => {
    if (roomDetails.roomPhotoUrls && roomDetails.roomPhotoUrls.length > 0) {
      setCurrentImageIndex((prevIndex) => 
        prevIndex === 0 ? roomDetails.roomPhotoUrls.length - 1 : prevIndex - 1
      );
    }
  };
  ...
  const { roomType, roomPrice, roomPhotoUrls, description, bookings } = roomDetails;
  ...
  return (
    <div className="room-details-container">
      ...
      <h2>Room Details</h2>
      <br />
      <div className="room-image-carousel room-details-form">
        {roomPhotoUrls && roomPhotoUrls.length > 0 ? (
          <>
            <img 
              src={`${ApiService.BASE_URL}${roomPhotoUrls[currentImageIndex]}`}
            ...
            />
            {roomPhotoUrls.length > 1 && (
              <div className="carousel-controls">
                <button className="carousel-button prev" onClick={prevImage}>
                  <FaChevronLeft />
                </button>
                <div className="image-indicators">
                  {roomPhotoUrls.map((_, index) => (
                    <span
                      key={index}
